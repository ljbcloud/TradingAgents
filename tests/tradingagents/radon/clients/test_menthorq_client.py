"""Tests for tradingagents.radon.clients.menthorq_client module."""

from unittest.mock import MagicMock, patch

import pytest

from tradingagents.radon.clients.base import ClientStatus
...
def mq_client_unavailable():
    ...
    with (
        patch("tradingagents.radon.clients.menthorq_client.HAS_PLAYWRIGHT", False),
...
class TestMenthorQClientInit:
    ...
    @patch("tradingagents.radon.clients.menthorq_client.HAS_PLAYWRIGHT", True)
    def test_raises_auth_error_without_credentials(self):
        from tradingagents.radon.clients.menthorq_client import MenthorQAuthError
...
class TestMenthorQClientAvailability:
    def test_check_availability_available(self):
        ...
        with patch("tradingagents.radon.clients.menthorq_client.HAS_PLAYWRIGHT", False):
            client = MenthorQClient.__new__(MenthorQClient)
...
class TestMenthorQClientScreenerValidation:
    def test_screener_category_raises_on_invalid_category(self, mq_client_unavailable):
        from tradingagents.radon.clients.menthorq_client import MenthorQExtractionError
...
class TestMenthorQClientSummaryValidation:
    ...
    def test_summary_accepts_futures(self, mq_client_unavailable):
        ...
        with patch(
            "tradingagents.radon.clients.menthorq_client._scrape_tables",
            return_value=[],
        ):
...
class TestMenthorQClientGetEod:
    def test_get_eod_raises_on_empty_scrape(self, mq_client_unavailable):
        ...
        with (
            patch(
                "tradingagents.radon.clients.menthorq_client._scrape_eod_fields",
                return_value={},
            ),
...
class TestMenthorQSanitizeText:
    def test_collapse_whitespace(self):
        from tradingagents.radon.clients.menthorq_client import _sanitize_text
...
class TestMenthorQParseForexText:
    def test_parses_single_pair(self):
        from tradingagents.radon.clients.menthorq_client import _parse_forex_text
...
class TestMenthorQExceptionHierarchy:
    def test_base_error(self):
        from tradingagents.radon.clients.menthorq_client import MenthorQError

        err = MenthorQError("test")
        assert str(err) == "test"

    def test_auth_error_inherits(self):
        from tradingagents.radon.clients.menthorq_client import (
            MenthorQAuthError,
            MenthorQError,
        )
...
class TestMenthorQConstants:
    def test_cta_slugs_has_expected_keys(self):
        from tradingagents.radon.clients.menthorq_client import CTA_SLUGS
    ...
    def test_screener_slugs_has_expected_categories(self):
        from tradingagents.radon.clients.menthorq_client import SCREENER_SLUGS
    ...
    def test_summary_categories(self):
        from tradingagents.radon.clients.menthorq_client import SUMMARY_CATEGORIES

        assert {"futures", "cryptos"} == SUMMARY_CATEGORIES

    def test_dashboard_commands(self):
        from tradingagents.radon.clients.menthorq_client import DASHBOARD_COMMANDS
