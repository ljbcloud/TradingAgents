...
from tradingagents.radon.utils.atomic_io import atomic_save, verified_load
...
Path(STOCKS_DIR).mkdir(exist_ok=True, parents=True)
Path(OPTIONS_DIR).mkdir(exist_ok=True, parents=True)
...
def prune_cache(cache_dir: Path, max_files: int = 500) -> int:
    ...
    all_files.sort(key=operator.itemgetter(1))
    to_delete = len(all_files) - max_files
    return sum(1 for f, _ in all_files[:to_delete] if _safe_unlink(f))
